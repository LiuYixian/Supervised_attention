...
class SequenceGenerator(object):
    ...
    def _generate(
        ...
        **kwargs
        ...
        mean_attn, mean_attn_buf = None, None
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores):
            ...
            mean_attn_clone = mean_attn.index_select(0, bbsz_idx)[:, :, 1:step + 2] if mean_attn is not None else None
            ...
            for i, (idx, score) in enumerate(zip(bbsz_idx.tolist(), eos_scores.tolist())):
                ...
                def get_hypo():
                    ...
                    if mean_attn_clone is not None:
                        # remove padding tokens from attn scores
                        hypo_mean_attn = mean_attn_clone[i]
                    else:
                        hypo_mean_attn = None
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            # avg_attn_scores = avg_attn_scores[0][0][0]
            one_head_attn = (avg_attn_scores[0][0].squeeze(0) + avg_attn_scores[1][0].squeeze(0) )/2
            mean_head_attn = (avg_attn_scores[0][1].squeeze(0)  + avg_attn_scores[1][1].squeeze(0) )/2
            one_head_attn = avg_attn_scores[0][0].squeeze(0)
            mean_head_attn = avg_attn_scores[0][1].squeeze(0)
            all_one_head_attn.append(one_head_attn)
            all_mean_head_attn.append(mean_head_attn)
            ...
            if one_head_attn is not None:
                if attn is None:
                    attn = scores.new(bsz * beam_size, src_tokens.size(1), max_len + 2)
                    attn_buf = attn.clone()
                attn[:, :, step + 1].copy_(one_head_attn)

            if mean_head_attn is not None:
                if mean_attn is None:
                    mean_attn = scores.new(bsz * beam_size, src_tokens.size(1), max_len + 2)
                    mean_attn_buf = mean_attn.clone()
                mean_attn[:, :, step + 1].copy_(mean_head_attn)
            ...
            if len(finalized_sents) > 0:
                ...
                if mean_attn is not None:
                    mean_attn = mean_attn.view(bsz, -1)[batch_idxs].view(new_bsz * beam_size, mean_attn.size(1), -1)
                    mean_attn_buf.resize_as_(mean_attn)
            ...
            if mean_attn is not None:
                torch.index_select(
                    mean_attn[:, :, :step + 2], dim=0, index=active_bbsz_idx,
                    out=mean_attn_buf[:, :, :step + 2],
                )
            # swap buffers
            ...
            if mean_attn is not None:
                mean_attn, mean_attn_buf = mean_attn_buf, mean_attn
...
class EnsembleModel(torch.nn.Module):
    ...
    def _decode_one(
        ...
        attn = decoder_out[1]
        one_head_attn = attn['attn'][0]
        one_head_attn = one_head_attn.transpose(0, 1).chunk(1 + 2, 0)[1:]
        mean_attn = attn['attn'].mean(0)
        mean_attn = mean_attn.transpose(0, 1).chunk(1 + 2, 0)[1:]
        # one_head_attn = [a.cup() for a in one_head_attn]
        # mean_attn = [a.cup() for a in mean_attn]
        attn = [one_head_attn, mean_attn]
